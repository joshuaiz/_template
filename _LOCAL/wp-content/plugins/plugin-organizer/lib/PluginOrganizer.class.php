...
class PluginOrganizer {
	...
	function check_version() {
		...
		if (get_option("PO_version_num") != "7.2" && !in_array($pagenow, array("plugins.php", "update-core.php", "update.php"))) {
			$this->activate();
		}
	}
	...
	function activate() {
		...
		if (get_option("PO_version_num") != "7.2") {
			update_option("PO_version_num", "7.2");
		}
	...
	function update_post_status($newStatus, $oldStatus, $post) {
		...
		if ($postExists) {
			...
			if (get_post_type($post->ID) != 'plugin_filter') {
				...
				$permalink = $postSettings['permalink'];
			}
			...
			$wpdb->update($wpdb->prefix."po_plugins", array("status"=>$newStatus, "permalink"=>$permalink, "permalink_hash"=>md5($permalinkNoArgs), "permalink_hash_args"=>md5($permalink), "secure"=>$secure), array("post_id"=>$post->ID));
		}
	}
