...
<script language="javascript" type="text/javascript">
				...
				if (newElement.hasClass('pluginWrap')) {
					...
					if (jQuery(this).find(':hidden[value="'+newElement.find('.PO-group-id').val()+'"]').length == 0) {
						if (jQuery(this).prop('id') == 'PO-disabled-mobile-plugin-wrap') {
							...
							if (jQuery.inArray(newElement.find('.PO-plugin-id').val(), globalPlugins['mobile_groups']) > -1) {
							...
							if (jQuery.inArray(newElement.find('.PO-plugin-id').val(), globalPlugins['std_groups']) > -1) {
								newElement.addClass('globalGroupWrap');
							}
	...
	function PO_add_all(sourceType, targetType) {
	...
	function PO_remove_all(sourceType, targetType) {
		jQuery('#PO-disabled-'+targetType+'-plugin-wrap .'+sourceType+'Wrap').each(function() {
			if (targetType == 'std') {
